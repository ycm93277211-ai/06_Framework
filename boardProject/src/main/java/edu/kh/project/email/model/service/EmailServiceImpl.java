...
public class EmailServiceImpl implements EmailService {
	...
	// SpringTemplateEngine : 타임리프를 이용해서 html코드 -> java코드 변환
	...
	public String sendEmail(String type, String email) {
		...
		// map으로 보내는 이유 마이바틱스는 1개 보낼 수 있기에 
		// map으로 묶어 보내야함
		Map<String, String> map = new HashMap<>();
		map.put("authKey", authKey);
		map.put("email", email);
		
		// DB 저장시도 -  실패 시 해당 메서드 종료
		...
		try {
			...
			helper.setSubject("[boardProject] 회원가입 인증번호 입니다"); // 받는 사람
	...
	// 인증키와 이메일을 DB에 저장하는 메서드(insert를 하겠다)
