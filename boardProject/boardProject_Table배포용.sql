...
-- 로그인 SQL
	SELECT MEMBER_NO, MEMBER_EMAIL, MEMBER_PW, MEMBER_NICKNAME, MEMBER_TEL, MEMBER_ADDRESS, PROFILE_IMG, AUTHORITY, -- 'AUTHORIY' -> 'AUTHORITY'
...
CASE 
	WHEN SYSDATE - BOARD_WRITE_DATE < 1 / 24 / 60 -- 60초 이내
	THEN FLOOR((SYSDATE -BOARD_WRITE_DATE) * 24 * 60 * 60) || '초 전'
	
	WHEN SYSDATE - BOARD_WRITE_DATE < 1 / 24 -- 1시간 이내
	THEN FLOOR((SYSDATE -BOARD_WRITE_DATE) * 24 * 60) || '분 전'
	
	WHEN SYSDATE - BOARD_WRITE_DATE  < 1 -- 24시간 지나지 않음
...
SELECT LEVEL, C.* FROM
		...
		WHERE COMMENT_DEL_FL = 'N' -- 댓글이 삭제되지 않았거나 OR  자식 댓글 중 삭제되지 않은 행이 하나라도 있으면 부모 댓글을 조회 대상 포함
...
SENDER_NO,CHATTING_ROOM_NO ,
TO_CHAR(SEND_TIME,'YYY.MM.DD HH24:MI') SEND_TINE
FROM MESSAGE
