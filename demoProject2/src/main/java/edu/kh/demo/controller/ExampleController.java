...
@Controller
@RequestMapping("example") // /example 로 시작하는 모든 요청 매핑
...
public class ExampleController {
	...
	public String ex1(HttpServletRequest req, Model model) {
		...
		stdList.add(new Student("44444","김사번",40));
	...
	public String ex2(Model model) {
		...
		model.addAttribute("str","<h1> 테이스 중 &times; </h1>");
		
		
		return"example/ex2";
	}
	
	@GetMapping("ex3") // /example/ex1 Get 방식 요청 매핑
	...
	public String pathVariableTest(@PathVariable("path")int path) {
		...
		// 주소 중 {path} 부븐이 값을 가져와서 매개변수로 저장
