...
function getCompleteCount(){
  ...
  .then(response => response.text()) // 축약 형태
  .then(result =>{
...
addBtn.addEventListener("click" , ()=>{
  ...
  // - 요청 주소 : "/ajax/add"
  // - 데이터 전달방식 : post
  // - 전달 데이터(파리마터)  : todoTitle값 , todoContent값
  // JS <-> Java
  // JSON(JavaScript Object Notation) : 데이터를 표현하는 문법
  // 즉, JS<->Java 둘 사이 언어 해석(통역사 느낌)
   /*
    { "name" : "홍길동",
     "age" : 20
    }
  */
  ...
  // todoTitle 과 todoContent를 저장할 JS 객체 만들기
  const param = {
    // key : value
  ...
  .then( result => {
    ...
    if(result > 0){
      ...
      // 추가 성공해따면 작성했던 제목, 내용인풋  비우기
      todoTitle.value = "";
      todoContent.value = "";

      // 할 일이 새롭게 추가되었으므로
      // -> 전체 todo갯수 조회하는 함수 재호출
      getTotalCount();
...
// 비동기로 할 일 전체 목로을 조회 & html 화면에 출력까지 하는 함수
const selectTodoList = () => {

  fetch("/ajax/selectList",{method: "POST"})
  .then(resp => resp.json()) // 응답 결과를 json으로 받음
  .then( todoList =>{
    // 매개변수todoList :
    // 첫번째 then에서 resp.text() / resp.json() 했냐에 따라
    // 단순 텍스트이거나,JS Object 일 수 있음
    // 즉, 값이 여러개 리스트 ,맵 등으로 오면 JSON 이용 하여 쉽게 값을 빼올 수 있음
...
// popuplayer의 X 클릭 시 popuplayer 보이게 하기
popupClose.addEventListener("click",()=>{
  // display:non 처리 해주는 class 추가
...
deleteBtn.addEventListener("click",()=>{
  ...
  fetch("/ajax/delete",{
    ...
    headers : {"Content-Type" : "application/json"},
    body : todoNo
    // 보내는 데이터가 한개만 있을때todoNO 만 적어도 가능 body : todoNO 이런 식으로 가능
  })
  .then(resp => resp.text())
  .then(result => {
    if(result > 0){
...
changeComplete.addEventListener("click", ()=>{
  ...
  .then(result =>{
    if(result>0){
      ...
      // -> 서버 부하가 큼 servletTOdo();

      // 상세 조회 팝업에서 Y/N 바꾸기
      popupComplete.innerText = complete;
      
      //getCompleteCount(); 서버 부하가 큼
      // 기본 완료된 Todo 개수 +-1
...
// 수정 레이어에서 취소 버튼 클릭 시
updateCancel.addEventListener("clicke",()=>{
...
updateBtn.addEventListener("click",(e)=>{
  ...
  const obj = {
    "todoNo" : e.target.dateset.todoNo,
